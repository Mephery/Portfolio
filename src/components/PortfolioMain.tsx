...
function PChaos()      {
  ...
  return <><p style={{...body,marginBottom:'1.5rem'}}>Construit de bout en bout — infra Proxmox (2 LXC prod/dev), Docker Compose (Postgres + Nginx), jusqu'au frontend. E2E Double Ratchet, WebRTC avec IA suppresseur de bruit, modération, XSS/SQLi.</p><p style={subLabel}>Stack Chaos</p><div style={{marginBottom:'1.5rem'}}>{s1.map(s=><span key={s} style={{...tagStyle,borderColor:'rgba(255,60,80,0.2)',color:'rgba(255,130,145,0.85)'}}>{s}</span>)}</div><p style={subLabel}>Talos — supervision & admin</p><div>{['React','Tailwind','Node.js'].map(s=><span key={s} style={tagStyle}>{s}</span>)}</div></>;
}
function PSkills()     {
...
function PSchool()     {
  return <div style={{display:'grid',gap:'1.25rem'}}>{[1,2,3].map(n=><div key={n} style={{padding:'1.25rem',border:'1px solid rgba(0,140,255,0.1)',background:'rgba(0,15,40,0.5)'}}><p style={{fontWeight:600,color:'#ddeeff',marginBottom:'0.5rem'}}>Projet BTS {n}</p><p style={{...body,color:'rgba(200,220,255,0.55)',fontSize:'0.85rem'}}>[Description du projet, contexte, ce que vous avez développé.]</p></div>)}</div>;
}
function PContact()    {
  const l=[{h:'mailto:coline.derycke@gmail.com',t:'coline.derycke@gmail.com'},{h:'#',t:'GitHub — à compléter'},{h:'#',t:'LinkedIn — à compléter'}];
  return <><p style={{...body,marginBottom:'2rem'}}>[Votre message d'accroche — disponibilité, ce que vous cherchez.]</p>{l.map(({h,t})=><div key={t} style={{marginBottom:'0.8rem'}}><a href={h} style={{...mono,fontSize:'0.68rem',letterSpacing:'0.3em',color:'rgba(0,185,255,0.75)',textDecoration:'none'}} onMouseEnter={e=>e.currentTarget.style.color='rgba(255,50,80,0.9)'} onMouseLeave={e=>e.currentTarget.style.color='rgba(0,185,255,0.75)'}>▸ {t.toUpperCase()}</a></div>)}</>;
}
